...
In the picture below there is a Target File example where the mandatory columns are enlightened in yellow and the optional column in green. You can fill the Target File with any other informative column, just try to avoid special characters like \\^$?\*/|+()[]-{} and whitespace. You can use **\_** instead of whitespace. For example, a column "Bio_replicate" may contain values like "BD1" or "BD_1" but not "BD-1" (Differential Analysis doesn't work if there are "-"). 
...
\***nomenclature of LipidSearch:** LipidSearch nomenclature is very similar to Lipid Maps nomenclature except for the using of underscore instead of backslash between different stereospecific numbering (sn) and the absence of the double-bond geometry. An example of the use of the nomenclature can be seen in the picture below.
